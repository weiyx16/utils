function vertface2obj_tex(fv,file_name)
...
name = strcat(file_name, '_texture.obj');
fid = fopen(name,'w');

[n,~] = size(fv.vertices);
for i=1:n %14772
    fprintf(fid,'v %f %f %f %f %f %f\n',fv.vertices(i,1),fv.vertices(i,2),fv.vertices(i,3),...
        double(fv.FaceVertexCData(i,1))/255.0,double(fv.FaceVertexCData(i,2))/255.0,double(fv.FaceVertexCData(i,3))/255.0);
end
